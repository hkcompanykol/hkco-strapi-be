    ...
    try {
        ...
        for (const category of categories) {
            ...
            const parentId = category.parent
                ? categoryBySlug[category.parent]?.id
                : null;
            const created = await app.entityService.create(
                "api::category.category",
                {
                    data: {
                        name: category.name,
        ...
        for (const [index, product] of products.entries()) {
            const exists = await app.entityService.findMany(
                "api::product.product",
                {
        ...
        console.log(
            "Seed complete: categories, brands, and products created/verified."
        );
    } catch (error) {
